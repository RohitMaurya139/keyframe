...
// Landing is the exact design export (public). The STUDIO (create → understanding
// → script → theater → premiere) requires login — an unauthenticated visitor who
// tries to create is sent to the Auth screen, then resumed. Gallery stays public.
export default function App() {
  ...
  const startGeneration = useCallback(({ prompt, url }) => requireAuth(() => runGenerate({ prompt, url }), "signup"), [requireAuth, runGenerate]);
      ...
      <div className="fixed inset-0">
        ...
        {starting && (
          <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ background: "rgba(233,230,221,0.85)", backdropFilter: "blur(6px)" }}>
            <div className="text-center"><div className="eyebrow">Opening the aperture…</div>
              <div className="wordmark text-3xl mt-2"><span style={{ color: "var(--color-ink)" }}>KEY</span><span style={{ color: "var(--color-green)" }}>FRAME</span></div></div>
          </div>
        )}
      </div>
    );
  ...
  return (
    <div className="grain min-h-full flex flex-col">
      ...
      <header className="sticky top-0 z-50 flex items-center justify-between px-6 sm:px-10 py-4 border-b"
        style={{ borderColor: "var(--color-line)", background: "rgba(233,230,221,0.72)", backdropFilter: "blur(12px)" }}>
        <button onClick={() => go("landing")} className="flex items-center gap-2.5" aria-label="KEYFRAME home">
          <span className="relative inline-flex" style={{ width: 22, height: 22 }}>
            <svg width="22" height="22" viewBox="0 0 24 24" aria-hidden="true">
              ...
              {[0, 60, 120, 180, 240, 300].map((a) => (<line key={a} x1="12" y1="12" x2="12" y2="3" stroke="var(--color-ink)" strokeWidth="1" opacity="0.55" transform={`rotate(${a} 12 12)`} />))}
            </svg>
          </span>
        ...
        <nav className="hidden sm:flex items-center gap-7">
          ...
          <button className="label-mono" style={{ color: view === "create" ? "var(--color-accent-text)" : "var(--color-dim)" }} onClick={() => enterStudio("create")}>Studio</button>
        ...
        {/* auth controls */}
        {loading ? <span className="label-mono" style={{ color: "var(--color-dim)" }}>…</span>
          : user ? (
            <div className="flex items-center gap-3">
              <span className="hidden sm:flex items-center gap-2">
                <span className="inline-flex items-center justify-center rounded-full font-display font-bold text-[13px]"
                  style={{ width: 30, height: 30, background: "var(--color-lime)", color: "var(--color-ink-2)" }}>
                  {(user.name || user.email || "?").trim().charAt(0).toUpperCase()}
                </span>
                <span className="label-mono" style={{ color: "var(--color-ink)" }}>{(user.name || user.email).split(" ")[0]}</span>
              </span>
              <button className="btn-ghost" style={{ padding: "8px 16px", fontSize: 13 }} onClick={() => { logout(); go("landing"); }}>Log out</button>
            </div>
          ) : (
            <div className="flex items-center gap-2.5">
              <button className="btn-ghost" style={{ padding: "9px 16px", fontSize: 13 }} onClick={() => { setAuthMode("login"); pending.current = () => go("create"); setView("auth"); }}>Log in</button>
              <button className="btn-lime" style={{ padding: "10px 18px", fontSize: 13 }} onClick={() => { setAuthMode("signup"); pending.current = () => go("create"); setView("auth"); }}>Sign up</button>
            </div>
          )}
      </header>
